...
// Nominal symbol timings in microseconds. Verified against 9664 measured
// symbols with zero outliers.
static const uint32_t MARK_SHORT_US = 288;
static const uint32_t MARK_LONG_US = 800;
static const uint32_t SPACE_SHORT_US = 224;
static const uint32_t SPACE_LONG_US = 704;

// Match slack, percent. Do not tighten: the AGC skews the duty cycle over the
// first frame of a burst, and the short space alone spans 136-312us -- a 39%
// inherent spread. Anything under ~40% rejects real frames. The two symbol
// classes sit a clean 3:1 apart, so 40% stays unambiguous.
static const uint32_t TOLERANCE_PCT = 40;
...
inline bool timing_matches(int32_t value, uint32_t nominal) {
  ...
  return v >= (100 - TOLERANCE_PCT) * nominal / 100 && v <= (100 + TOLERANCE_PCT) * nominal / 100;
}
...
inline bool decode_frame(const int32_t *frame, size_t len, uint32_t *out) {
  ...
  for (size_t i = 0; i < PACKET_BITS * 2; i += 2) {
    ...
    if (timing_matches(mark, MARK_LONG_US) && timing_matches(space, SPACE_SHORT_US)) {
      bits = (bits << 1) | 1;
    } else if (timing_matches(mark, MARK_SHORT_US) && timing_matches(space, SPACE_LONG_US)) {
      bits = bits << 1;
    } else {
      return false;
    }
...
// Split a capture on inter-frame gaps, decode every chunk independently, and
// majority-vote the ones that decoded.
//
...
inline DecodeResult decode_capture(const int32_t *data, size_t len) {
  ...
  for (size_t i = 0; i <= len; i++) {
    ...
    result.frames_total++;
    uint32_t bits;
    if (i > start && decode_frame(&data[start], i - start, &bits)) {
